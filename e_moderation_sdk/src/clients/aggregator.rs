...
impl SlashAggregator {
    ...
    pub fn reconstruct_strike(
        ...
        if certificates.len() < self.n_moderator_threshold as usize {
            return Err(
                "The number of certificates has not yet reached the N-of-M moderator threshold.",
            );
        }
        ...
        for cert in certificates {
            let pubkey_bytes: [u8; 32] = cert
                .moderator_pubkey
                .as_slice()
                .try_into()
                .unwrap_or([0; 32]);

            if !self.valid_moderator_pubkeys.contains(&pubkey_bytes) {
                continue;
            }

            let message_hash = Self::hash_for_signature(tracing_tag, &cert.decrypted_share);
            let pubkey_obj =
                PublicKey::try_new(pubkey_bytes).map_err(|_| "Invalid Moderator Pubkey")?;
            ...
            if !seen_moderators.insert(pubkey_bytes) {
                continue;
            }
    ...
    pub fn reconstruct_nsk(
        ...
        for (x_index, s_post) in accumulated_strikes
            .iter()
            .take(self.k_strikes_threshold as usize)
        {
